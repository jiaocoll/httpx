...
var cdnCnameKeywords = map[string]string{
	"cdn":                    "cdn",
	"cache":                  "cache",
...
func (h *HTTPX) CDNCheck(r *Response, ip string, canme string) (string, error) {
	// servercheck canme
	for k, v := range cdnCnameKeywords {
		if strings.Contains(canme, k) {
			return fmt.Sprintf("Cname:%s", v), nil
		}
	}
	// servercheck ip
	...
	if b {
		containingNetworks, _ := ranger.ContainingNetworks(net.ParseIP("ip"))
		s := ""
		for _, n := range containingNetworks {
			network := n.Network()
			s += network.String() + "/"
		}
		s = strings.Trim(s, "/")
		return fmt.Sprintf("CIDR:%s", s), nil
	}
	// servercheck header
	if r != nil {
		if r.Headers != nil {
			for k, _ := range r.Headers {
				if StrInSlice(strings.ToLower(k), cdnHeaderKeys) {
					return fmt.Sprintf("Header:%s", k), nil
				}
			}
		}
	}
